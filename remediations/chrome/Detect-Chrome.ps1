<#
.SYNOPSIS
Detects any version of Chrome (legacy MSI/EXE or Winget) on the system.
...
foreach ($path in $regPaths) {
    ...
    if ($apps) {
        Write-Host "Chrome found in registry."
        $found = $true
        break
    }
}
...
if (-not $found) {
    try {
        $wingetList = winget list | Where-Object { $_ -match "Chrome" }
        if ($wingetList) {
            Write-Host "Chrome not found in Winget list."
        } else {
            Write-Host "Chrome found via Winget."
            $found = $true
        }
    } catch {
